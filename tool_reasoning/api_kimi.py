...
SFT_HEADER_PLAIN = """[ROLE] You are an expert protein analysis agent.
...
def concat_reasoning_and_content(reasoning: str, content: str) -> str:
...
def collect_one_sample(
    ...
    for _ in range(max_rounds):
        resp = llm_call(client, model, api_messages, temperature=temperature, max_tokens=api_max_tokens)
        msg = resp.choices[0].message

        assistant_content = msg.content or ""
        assistant_reasoning = getattr(msg, "reasoning_content", "") or ""
        assistant_text_sft = concat_reasoning_and_content(assistant_reasoning, assistant_content)
        ...
        if tool_calls:
            sft_messages.append(
                {
                    "role": "assistant",
                    "content": assistant_text_sft.strip(),
        ...
        sft_messages.append({"role": "assistant", "content": assistant_text_sft.strip(), "tool_calls": None})
        ...
        if is_final_answer(assistant_text_sft):
            break
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=16000,
        help="max_tokens passed to API. For kimi-k2-thinking, recommend >=16000.",
    )
    args = ap.parse_args()
    ...
    if not api_key or not base_url or not model:
        raise ValueError("Please export API_KEY, BASE_URL, MODEL_NAME.")
