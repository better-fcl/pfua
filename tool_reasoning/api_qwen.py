...
def get_merged_assistant_text(msg: Any) -> str:
    """
    Merge provider-specific reasoning (if any) into content to ensure non-empty text.
    This merged text will be used BOTH:
      - for saving to jsonl (sft_messages)
      - for echoing back into api_messages
    """
...
def collect_one_sample(
    ...
    include_reasoning_field = os.environ.get("INCLUDE_REASONING_FIELD", "0").strip() in ("1", "true", "True")
    ...
    for _ in range(max_rounds):
        ...
        merged_text = get_merged_assistant_text(msg)
        merged_text_stripped = (merged_text or "").strip()

        if tool_calls:
            sft_messages.append(
                {
                    "role": "assistant",
                    "content": merged_text_stripped,
            ...
            if include_reasoning_field:
                assistant_payload["reasoning_content"] = getattr(msg, "reasoning_content", "") or ""
            ...
            for tc in tool_calls:
                ...
                tool_json, _tool_meta = run_tool_via_bash(
        ...
        sft_messages.append({"role": "assistant", "content": merged_text_stripped, "tool_calls": None})

        assistant_payload = {"role": "assistant", "content": merged_text}
        if include_reasoning_field:
            assistant_payload["reasoning_content"] = getattr(msg, "reasoning_content", "") or ""
        api_messages.append(assistant_payload)

        if is_final_answer(merged_text):
            break

    sft_record = {"messages": sft_messages, "tools": TOOLS_SPEC}
    return sft_record
...
def main():
    ...
    if not api_key or not base_url or not model:
        raise ValueError("Please export API_KEY, BASE_URL, MODEL_NAME (for Qwen/DashScope compatible-mode).")
    ...
    for i, item in enumerate(data):
        try:
            ...
            sft_record["gold_output"] = item.get("output", None)
            sft_record["source_metadata"] = item.get("metadata", {})
            sft_record["source_item_index"] = i
